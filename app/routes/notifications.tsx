...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (actionType === "accept") {
    ...
    await db("users").where("userId", partnerUserId).update({ partnerId: null });
    await db("users").where("userId", user.userId).update({ partnerId: partnerUserId });
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  let partnerData;
  const partner = await db.select("partnerId").from("users").where("userId", user["userId"]);
  try {
      ...
      <div className="flex flex-col items-center">
          ...
          <div>
            {partnerData.map((request) => (
              ...
              <RequestButton text="Accept" onClick={() => handleAccept(request.userId)} styleType="accept" />
              <RequestButton text="Deny" onClick={() => handleDeny(request.userId)} styleType="deny" />
              </div>
            ))}
          </div>
        </div>
